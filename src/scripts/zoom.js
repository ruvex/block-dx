...
const showZoomLevel = percent => {
  const id = uuid.v4();
  const styles = 'background-color:#fff;color:#000;width:70px;height:30px;line-height:30px;font-size:14px;text-align:center;position:fixed;right:0;top:0;z-index:10000;';
  const className = 'js-zoomLevelContainer';
  $(`.${className}`).remove();
  $('body').append(`<div id="${id}" class="${className}" style="${styles}">${percent}%</div>`);
  setTimeout(() => {
    $(`#${id}`).remove();
  }, 2000);
};
...
ipcRenderer.on('ZOOM_IN', () => {
  ...
  if(zoomFactor < maxZoom) {
    ...
    showZoomLevel(newZoomFactor);
  }
});
ipcRenderer.on('ZOOM_OUT', () => {
  const zoomFactor = getZoomFactor();
  if(zoomFactor > minZoom) { // zoom out
    const newZoomFactor = zoomFactor - zoomIncrement;
    setZoomFactor(newZoomFactor);
    showZoomLevel(newZoomFactor);
  }
